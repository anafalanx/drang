...
## Decision + build: persistent JSON store (2026-07-05)

Programming languages have no memory between runs: a glue/orchestration script that runs on a schedule re-derives everything each time. The canonical need — remember a cursor / checkpoint / accumulator / cache across invocations — is squarely drang's niche. This ships a **store**: a durable JSON key-value map behind a small, curated builtin surface.

**Rejected: an embedded database.** SQLite via `modernc.org/sqlite` was feasible (pure-Go, single static exe preserved) but costs ~+4 MB and takes go.sum from 1→10 modules, and SQL is a second stateful mini-language whose engine is exactly the third-party dependency the TOML/YAML gate exists to stop (dependency-light pillar). Rejected in favor of hand-rolling on drang's own JSON codec: zero new dependency, on-grain (values are maps/arrays, errors-as-values), and the store *is* a drang value. bbolt / buntdb / ncruces-WASM-SQLite were surveyed and set aside for the same reason — one persistence primitive, the smallest that fits the need.

**Engine: atomic snapshot per write.** Every mutation rewrites the whole file (temp + fsync + atomic rename via `MoveFileEx` REPLACE_EXISTING), keeping the previous copy as `<path>.bak`. The simplest thing that is fully crash-consistent: the file is never observed torn; a crash leaves either the old or the new complete snapshot. An append-log + compaction (O(1) writes) was considered and deferred — a clean later upgrade with no on-disk format break; atomic-snapshot is the cautious floor.

**One writer, cross-process.** A store holds a process-exclusive advisory lock (`LockFileEx`, fail-immediately) on a *separate* `<path>.lock` for its lifetime; a second process opening the same store gets a catchable `store busy` Err. Locking a sidecar file (never the data file) keeps the atomic rename unhindered and lets other tools read the data. So `store_update`'s cross-process atomicity is free from the lock (only one run mutates at a time); within a process it is the store mutex, held across the (pure) update lambda.

**Location: env-var-free, script-adjacent.** `store()` with no path defaults to `.drang/<script>.store` in the running script's directory — predictable, travels with the script, and never derived from `%LOCALAPPDATA%`/`%APPDATA%`/roaming/OneDrive (an explicit anti-goal: the multi-level, OneDrive-redirected settings mess of other Windows tools). The runtime itself keeps **no** state — drang stays a clean static binary that writes nothing behind your back. `-e`/stdin have no script file, so they pass an explicit path. There is no user-global or shared store — `store()` and `store("path")` only.

**Surface.** `store` (special form — needs env for the default path), `store_get`/`store_set`/`store_has`/`store_delete`/`store_keys`/`store_all`/`store_clear`/`store_path`/`store_close` (plain builtins), `store_update(s, key, default, fn)` (special form — atomic read-modify-write; the `default` is what `fn` sees when the key is absent, before the lambda, mirroring `reduce`), `with_store(s, fn)` (special form — all-or-nothing batch, rolls back on Err). Keys are strings; values any JSON-serializable drang value (a channel/task/process/function → catchable Err). Soft 64 MiB cap. `store_get` on an absent key is `nil` (or a supplied default), like map access — not an Err.

**Concurrency.** The handle is a shared reference like a channel (`DeepCopy` returns itself), mutex-guarded, safe to pass to `spawn`/`pmap` — access is serialized, not a parallelism win. Values are deep-copied in and out (copy-on-send), so a stored container can't be mutated across goroutines. `store_update` holds the mutex across its pure lambda (atomic RMW); `with_store` does **not** hold it across the batch lambda (which re-enters the store), using a pre-batch snapshot for rollback.

**Verification.** The new names dispatch identically on the register VM and the tree-walker through the shared `builtins` map / `dispatchNonUser` seam — no fallback. Hand-written `TestVMParity` cases (the fuzzer's `pureProgram` allowlist skips all store names by design — they touch disk), plus a `-race` concurrency stress and a default-path test. Full `go test -race ./...` green.

**Deferred (clean later additions, no format break):** the append-log + compaction engine; nested-path keys (`store_get(s, ["a","b"])`); a user-global store if a real, demonstrated need appears.

## Decision + build: `recv_stdout` — live child stdout reading (2026-07-05)

Completes single-process bidirectional steering. Before this a started child could be *driven* on stdin (`start(…, {stdin_pipe: true})` + `send_stdin`/`close_stdin`) but its stdout was detached to the null device — you could talk to a subprocess but never watch it answer, except by routing it to a file and reading the file back. The 0.7 vocabulary freeze reserved the names `recv_stdout` and the `{stdout_pipe}` start-option for exactly this; 0.10.0 fills them additively, with no change to the frozen surface.

**Design.** `{stdout_pipe: true}` (start-only, mirroring `{stdin_pipe}`) keeps the parent read end of an `os.Pipe` on the `Proc`; the child writes the write end. Because that write end is handed to the launcher as an `*os.File`, `writerDescriptor`'s fast path attaches it directly (no draining copier goroutine), which means `builtinStart` must close **its own** copy of the write end after a successful launch — otherwise the parent read end never sees EOF on child exit. This is the same discipline `stream_lines` already follows for its input pipe. `recv_stdout(p)` is a plain map-builtin (so it gets automatic VM↔walker parity, like `send_stdin`) that does a **direct blocking `Read`** on the pipe and returns the raw chunk (untrimmed bytes), or `nil` at EOF, or a catchable `Err` (not piped / read failure); wrong arity aborts.

**Two decisions (polled).** (1) *Read mechanism = direct read, not a background drainer.* The script is the drainer; a child that outruns the read pace back-pressures, and `await`-ing a child that has filled an unread output pipe blocks — fundamental pipe semantics, the same "never wait on an undrained pipe" rule `stream_lines` documents (`kill(p)` always unwedges). Chosen over a background-drainer-into-a-bounded-buffer, which would add a goroutine + buffer + cap *per child* and a cap-and-drop data-loss failure mode, to defend against a misuse (`await` before draining) that is better handled by documentation than by machinery. (2) *Framing = raw bytes/chunks, not lines,* so a promptless prompt (no trailing newline) arrives immediately instead of a line reader stalling for the `\n`; `stream_lines` owns line-framing. EOF surfaces as `nil` (consistent with channel `recv`).

**Deadlock-guard note.** The `spawnedLive` deadlock guard fires only inside `Chan.recv`/`send`; a child process is not a drang task, so a channel-shaped stdout surface (`stdout_chan`) would false-trip it. `recv_stdout` being a *process-aware builtin* that reads the pipe directly never touches that guard — the concrete reason the surface is a builtin, not a channel.

**Honest limit.** Many programs block-buffer stdout when it is a pipe rather than a TTY, so their output appears only on flush/exit. Fully interactive steering of such programs needs a pseudo-terminal (ConPTY), which is deferred.

**Verification.** Parity (`assertBoth`) on a send-then-drain round-trip (`sort`), an EOF case (`echo` → chunk then `nil`), rejection cases (not-a-proc / not-piped → `Err`; `{stdout_pipe}` on a synchronous form / wrong arity → abort), and a 6-worker `-race` concurrency test over the per-`Proc` stdout state. Full `go test -race ./...` green.

## Decision + build: `recv_stderr` — the last single-process gap (2026-07-05)

With `recv_stdout` shipped, an audit of the single-process orchestration surface found exactly one genuine capability still missing: reading a started child's **stderr as a stream distinct from its stdout**. `{merge_stderr}` could fold stderr into stdout, and synchronous `capture_all` returns a separate `{err}` buffer, but a *live* started child's stderr was otherwise detached to the null device. Unlike `recv_stdout`/`{stdout_pipe}` and `stop`/`signal`, the names `recv_stderr`/`{stderr_pipe}` were **never reserved** in the 0.7 freeze — this is the first genuinely new name added under the additive-only contract (which permits adding names, never repurposing them).

**Why build it (when `stop`/`signal` were deferred the same day).** The build-on-real-need test that deferred graceful stop *passed* here: the gap was concrete and demonstrated (you literally could not read a live child's stderr), the design is a faithful mirror of an already-shipped, already-verified primitive (`recv_stdout`), and it adds no new kernel surface — just a second `os.Pipe`. Low risk, real need: build it.

**Design (a mirror).** `{stderr_pipe: true}` (start-only) keeps the parent read end of a second `os.Pipe` on the `Proc`; `recv_stderr(p)` is a plain map-builtin (automatic VM↔walker parity) that direct-reads it — raw chunks, `nil` at EOF, catchable `Err` if not piped, aborting on wrong arity. `readStderr`/`closeStderr` and the per-`Proc` state (`stderrMu`/`stderrR`/`stderrClosed`) duplicate the stdout path exactly, with a **separate mutex** so the two readers never contend. `builtinStart` closes its own copy of the child's stderr write end after launch (the `writerDescriptor` `*os.File` fast path does not track it in `childFiles`), so the read end EOFs on child exit.

**Mutual exclusion with `{merge_stderr}` (closing a silent-starve hole).** `childStderr` routes stderr to the stdout descriptor when `merge_stderr` is set — which, combined with `{stderr_pipe}`, would leave the stderr pipe with no writer and make `recv_stderr` return an immediate empty EOF. Rather than silently mislead, `{stderr_pipe}` + `{merge_stderr}` together is rejected at option-parse time (an uncatchable usage abort, like the other option-validation errors). You fold stderr in *or* read it apart, not both.

**The two-pipe deadlock — documented, not engineered around.** stdout and stderr are independent pipes: a script that drains only one while the child fills the other will wedge (the child blocks on the full pipe; the script blocks reading the empty one). The remedy is the classic one — **drain concurrently**, reading one stream in a `spawn`ed task — which drang expresses natively; MANUAL and REFERENCE show the pattern and name the hazard. (A select-style "read whichever is ready" primitive was weighed and rejected as over-scope; `{merge_stderr}` already serves the "I just want everything, in order" case with no deadlock risk.)

**Verification.** An adversarial review across three independent lenses (pipe lifecycle, race safety, semantics/merge/parity) confirmed the mirror on every axis and surfaced one low-severity *pre-existing* defect: on an `os.Pipe` *creation* failure with `{stdin_pipe}` set, the stdin read end leaked to GC instead of being explicitly closed — now fixed in both the stdout and stderr failure handlers. Tests: cross-backend `assertBoth` read + EOF + rejection cases, a deterministic separate-streams test, a concurrent stdout+stderr drain, and a 6-worker `-race` stress over the per-`Proc` stderr state. Full `go test -race ./...` green.

## Decision: graceful process stop deferred (2026-07-05)

Graceful `stop`/`signal` — let a child clean up before dying, versus the hard tree-terminate `kill` provides — is **deferred**: decision-record only, not built. The reserved names `stop`/`signal` (0.7 freeze) stay reserved.

**Why deferred, not built.** (1) *Speculative.* No demonstrated daily-driver need has surfaced, unlike `recv_stdout`, which closed a real capability hole (you genuinely could not read a live child). Building the riskiest piece of the orchestration set for an unvoiced need violates the build-on-real-need discipline. (2) *Worst value-to-risk in that set.* It is net-new kernel surface: `CREATE_NEW_PROCESS_GROUP` at spawn, `GenerateConsoleCtrlEvent` (not in `x/sys` — a hand-declared syscall), and it interacts with drang's own console control handler. (3) *It cannot honestly keep the promise its name implies.* `CTRL_BREAK` targets a process **group** by pid, not the job **tree**: it reaches the direct child (the group leader) but not nested descendants, so a `stop` advertised as "gracefully stop the child" that then hard-kills the whole tree on timeout over-promises — precisely the kind of surprise drang works to avoid. (4) *It belongs to supervision.* Graceful, ordered shutdown of a subtree is a supervisor concern; its natural home is the future `sturm` supervision tree, not a bolted-on single-process `stop` that would likely be redesigned when `sturm` lands.

**Reopen when:** a concrete need to gracefully stop a specific, well-behaved (`CTRL_BREAK`-handling) child appears, or the `sturm` tree is built (its supervisors own graceful termination). Until then `kill(p)` (hard tree-terminate) is the stop mechanism, and a poor-man's graceful stop is available where the child reads stdin: `send_stdin` a quit command, then a timeout + `kill`.

## Decision: ConPTY (pseudo-terminal) deferred (2026-07-05)

A **pseudo-terminal** — the Windows `CreatePseudoConsole` (ConPTY) API, the OS-level fake terminal that Windows Terminal, VS Code, and modern OpenSSH use — would let `start` hand a child a real console instead of a plain pipe, so the child's `isatty()` reads true. It is **deferred**: decision-record only, not built. drang's honest limit stands ("a full pseudo-terminal (ConPTY) is not provided").

**What it would buy.** Two things the current plain-pipe `{stdout_pipe}`/`{stderr_pipe}` cannot: (1) programs that **block-buffer** when their output is a pipe (many do, detecting a non-TTY) would instead flush live, so `recv_stdout` sees output as it is produced, not only on exit; (2) genuinely **interactive** children that gate on a terminal — password/passphrase prompts (`ssh`, `git` credentials), REPLs (`python`/`node`/`psql`), pagers, menu-driven installers — become drivable, extending "drive a live child" from cooperative batch tools to arbitrary interactive ones.

**Intended shape (when built).** Opt-in, additive, and **raw-VT-first**: a `{pty: true}` start-option that swaps the plain-pipe wiring for a ConPTY, with `send_stdin`/`recv_stdout` carrying the byte stream. The existing pipe model is untouched — `{pty}` is a heavier second mode. `recv_stdout` over a ConPTY returns the **raw VT stream honestly** (ANSI/xterm escape sequences plus conhost's repaints), and the caller strips it; drang does NOT ship a terminal-emulator / clean-text layer in the first cut.
...
drang development stops here for now. Single-process orchestration is **complete** as of 0.10.0 (`start` → `send_stdin`/`close_stdin` → `recv_stdout`/`recv_stderr` → `max_*`/`timeout` → `kill`/`{supervise}`). The remaining known pieces — the `sturm` supervision tree, ConPTY, and graceful `stop`/`signal` — are all deferred behind a **demonstrated daily-use need**, each with its own decision-record above. The next move is driven by real-world use of drang, not by the roadmap: when a concrete need surfaces (a supervised subtree to shut down in order; an interactive child to steer; a well-behaved child to stop gracefully), the corresponding record says how to reopen it.
